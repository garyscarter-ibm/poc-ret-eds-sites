...
const SVG_BMW_LOGO =
  '<svg viewBox="0 0 64 64"><circle cx="32" cy="32" r="30" fill="none" stroke="#000" stroke-width="3"/><path d="M32 2A30 30 0 0 1 62 32H32V2z" fill="#0066B1"/><path d="M32 2A30 30 0 0 0 2 32h30V2z" fill="#fff"/><path d="M32 62A30 30 0 0 1 2 32h30v30z" fill="#0066B1"/><path d="M32 62A30 30 0 0 0 62 32H32v30z" fill="#fff"/></svg>';
...
function navigateToDetail(vehicle) {
  ...
  const detailPath =
    blockEl.dataset.detailPage || "/used-cars/vehicle-showcase-detail";
...
function buildThumbnails() {
  ...
  vehicles.forEach((v, i) => {
    const thumb = el(
      "button",
      `vs-thumb ${i === currentIndex ? "vs-thumb--active" : ""}`,
    );
  ...
  if (progressEl) {
    ...
    vehicles.forEach((_, i) => {
      progressEl.appendChild(
        el("div", `vs-dot ${i === currentIndex ? "vs-dot--active" : ""}`),
      );
    });
  }
}
...
function buildFilterTabs(container) {
  ...
  allTypes.forEach((group, gi) => {
    ...
    if (group.type === "all") {
      const tab = el(
        "button",
        `vs-filter-tab ${activeFilter.type === "all" ? "vs-active" : ""}`,
        "All",
      );
      ...
      group.values.forEach((v) => {
        const isActive =
          activeFilter.type === group.type && activeFilter.value === v.value;
        const label =
          group.type === "fuelType" ? formatFuelType(v.value) : v.value;
        const tab = el(
          "button",
          `vs-filter-tab ${isActive ? "vs-active" : ""}`,
          label,
        );
...
export default async function decorate(block) {
  ...
  loading.innerHTML =
    '<div class="vs-loading-content"><p>Loading vehicles...</p><div class="vs-loading-bar"></div></div>';
  ...
  try {
    ...
    loading.innerHTML =
      '<div class="vs-loading-content"><p>Unable to load vehicles. Please try again.</p></div>';
    // eslint-disable-next-line no-console
