...
function createFilterDropdown(label, name, options, activeValues) {
  ...
  btn.addEventListener('click', () => {
    ...
    wrapper.closest('.vehicle-search-filters')?.querySelectorAll('.filter-dropdown-btn[aria-expanded="true"]').forEach((other) => {
      if (other !== btn) {
...
function renderFilterBar(facets, params, onFilterChange) {
  ...
  if (facets.series) {
    bar.append(createFilterDropdown('Series', 'series', facets.series, activeSeries));
  }
  if (facets.bodyTypes) {
    bar.append(createFilterDropdown('Body Type', 'bodyType', facets.bodyTypes, activeBody));
  }
  if (facets.fuelTypes) {
    bar.append(createFilterDropdown('Fuel Type', 'fuelType', facets.fuelTypes, activeFuel));
  }
  if (facets.transmissions) {
    bar.append(createFilterDropdown('Transmission', 'transmission', facets.transmissions, activeTrans));
  }
...
function renderSpecsGrid(vehicle) {
  ...
  const specs = [
    ...
    { iconName: 'transmission', text: formatTransmission(vehicle.transmission) },
...
function renderVehicleCard(vehicle, isSaved, onToggleSave) {
  ...
  heartBtn.setAttribute('aria-label', isSaved ? 'Remove from saved' : 'Save vehicle');
  heartBtn.innerHTML = `<img src="/icons/${isSaved ? 'heart-filled' : 'heart'}.svg" alt="" width="24" height="24">`;
...
export default async function decorate(block) {
  ...
  const handleToggleSave = async (vehicleId, btn) => {
    ...
    if (wasSaved) {
      ...
      btn.querySelector('img').src = '/icons/heart.svg';
    } else {
      savedIds.add(vehicleId);
      btn.classList.add('saved');
      btn.setAttribute('aria-label', 'Remove from saved');
      btn.querySelector('img').src = '/icons/heart-filled.svg';
    }

    // Fire API call in background
    try {
      if (wasSaved) {
        await queryAPI(REMOVE_GARAGE_MUTATION, { userId: toggleUserId, vehicleId });
      } else {
        await queryAPI(ADD_GARAGE_MUTATION, { userId: toggleUserId, vehicleId });
      }
    } catch {
      // Revert on failure
      if (wasSaved) {
  ...
  const fetchAndRender = async (params) => {
    ...
    try {
      ...
      filtersContainer.append(renderFilterBar(facets, params, (newParams) => {
        updateURL(newParams);
      ...
      vehicles.forEach((vehicle) => {
        ...
        const card = renderVehicleCard(vehicle, isSaved, handleToggleSave);
        results.append(card);
      });
      ...
      paginationContainer.append(renderPagination(page, totalPages, (newPage) => {
