...
export default async function decorate(block) {
  ...
  function updateCompareBar() {
    ...
    compareBar.querySelector(".compare-bar-text").textContent =
      `${count} vehicle${count !== 1 ? "s" : ""} selected for comparison`;
    compareBar.querySelector(".compare-bar-count").textContent = count;
    compareBar.querySelector(".compare-bar-btn").href =
      `/used-cars/compare?ids=${[...compareIds].join(",")}`;
    if (count >= 2) {
      compareBar
