...
const ICONS = {
  ...
  mileage: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/><path d="M12 6v6l4 2"/></svg>',
  gearbox: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="6" cy="6" r="2"/><circle cx="18" cy="6" r="2"/><circle cx="6" cy="18" r="2"/><path d="M6 8v10M18 8v4a2 2 0 01-2 2H8"/></svg>',
  engine: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M7 4v4m10-4v4M3 8h18v10a2 2 0 01-2 2H5a2 2 0 01-2-2V8z"/><path d="M7 12h4m2 0h4"/></svg>',
  registration: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="2" y="6" width="20" height="12" rx="2"/><path d="M6 12h.01M10 12h8"/></svg>',
  exterior: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="4"/></svg>',
  upholstery: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M4 18V8a4 4 0 014-4h8a4 4 0 014 4v10"/><path d="M4 14h16v4a2 2 0 01-2 2H6a2 2 0 01-2-2v-4z"/></svg>',
  location: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>',
  phone: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.362 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.338 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>',
  chevronLeft: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18L9 12L15 6"/></svg>',
  chevronRight: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18L15 12L9 6"/></svg>',
  fullscreen: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M8 3H5a2 2 0 00-2 2v3m18 0V5a2 2 0 00-2-2h-3m0 18h3a2 2 0 002-2v-3M3 16v3a2 2 0 002 2h3"/></svg>',
  ...
  heartFilled: '<svg width="24" height="24" viewBox="0 0 24 24" fill="#1b69d4" stroke="#1b69d4" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>',
  share: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>',
};
...
function renderGallery(images, accentColour) {
  ...
  function goTo(idx) {
    ...
    sideStrips.querySelectorAll('.vd2-gallery-strip').forEach((s) => {
      ...
      s.querySelector('.vd2-gallery-strip-overlay').style.opacity = sIdx === next ? '0' : '';
    });
  ...
  fsBtn.addEventListener('click', () => {
    ...
    lightbox.querySelector('.vd2-lightbox-prev').addEventListener('click', () => {
      lbIdx = (lbIdx - 1 + sorted.length) % sorted.length;
      lbImg.src = sorted[lbIdx].url;
    });
    lightbox.querySelector('.vd2-lightbox-next').addEventListener('click', () => {
...
function renderKeyFacts(vehicle) {
  ...
  const row1 = [
    { icon: ICONS.calendar, label: 'Registered', value: formatDate(vehicle.registrationDate) },
    { icon: ICONS.fuel, label: 'Fuel', value: formatFuelType(vehicle.fuelType) },
    { icon: ICONS.mileage, label: 'Mileage', value: formatMileage(vehicle.mileage) },
    { icon: ICONS.gearbox, label: 'Gearbox', value: formatTransmission(vehicle.transmission) },
  ];

  const row2 = [
    { icon: ICONS.engine, label: 'Engine Power', value: formatPower(vehicle.power) },
    { icon: ICONS.registration, label: 'Registration', value: vehicle.registrationNumber || '—' },
    { icon: ICONS.exterior, label: 'Exterior', value: vehicle.colour || '—' },
    { icon: ICONS.upholstery, label: 'Upholstery', value: vehicle.upholstery || '—' },
  ];
...
function renderPriceCard(vehicle, isSaved, onToggleSave, accentColour) {
  ...
  if (vehicle.estimatedMonthlyPayment) {
    card.append(el('div', 'vd2-monthly', `${formatMonthly(vehicle.estimatedMonthlyPayment)}  / mo`));
  }
  ...
  saveBtn.setAttribute('aria-label', isSaved ? 'Remove from saved' : 'Save vehicle');
...
export default async function decorate(block) {
  ...
  if (!vehicleId) {
    renderError(block, 'No vehicle ID provided. Please select a vehicle from the search results.');
    return;
  }
  ...
  try {
    ...
    if (vehicleData.status === 'rejected' || !vehicleData.value?.usedVehicle) {
      renderError(block, 'This vehicle could not be found. It may have been sold or removed.');
      return;
    }
  ...
  async function onToggleSave(btn) {
    ...
    btn.setAttribute('aria-label', currentlySaved ? 'Save vehicle' : 'Remove from saved');
    try {
      if (currentlySaved) {
