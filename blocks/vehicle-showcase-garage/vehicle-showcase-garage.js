...
function rebuildDots() {
  ...
  if (!indicator) return;
  indicator.innerHTML = garageVehicles
    .map(
      (_, i) =>
        `<div class="vsg-swipe-dot ${i === currentIndex ? "vsg-dot-active" : ""}"></div>`,
    )
    .join("");
}
...
async function transitionToVehicle(direction) {
  ...
  if (direction === "next") {
    ...
    nextIndex =
      currentIndex === 0 ? garageVehicles.length - 1 : currentIndex - 1;
  }
  ...
  const outClass =
    direction === "next" ? "vsg-sliding-out-left" : "vsg-sliding-out-right";
  ...
  const inClass =
    direction === "next" ? "vsg-sliding-in-left" : "vsg-sliding-in-right";
...
function buildGaragePage(block) {
  ...
  if (garageVehicles.length) {
    ...
    const dotsHtml = garageVehicles
      .map(
        (_, i) =>
          `<div class="vsg-swipe-dot ${i === 0 ? "vsg-dot-active" : ""}"></div>`,
      )
      .join("");
    indicator.innerHTML = `
  ...
  // Wire detail link
  actionBar
    .querySelector(".vsg-cta--secondary")
...
export default async function decorate(block) {
  ...
  block.innerHTML =
    '<div class="vsg-loading"><p class="vsg-loading-text">Loading your garage...</p></div>';
  ...
  try {
    ...
    block.innerHTML =
      '<div class="vsg-loading"><p class="vsg-loading-text">Failed to load your garage.</p></div>';
    // eslint-disable-next-line no-console
